...
{% highlight wl linenos %} struct Wanderer { explicit Wanderer(std::vector& update_loop) { update_loop.emplace_back(\[this\](float dt) { update(dt); }); } void update(float dt); }; struct Diver { explicit Diver(std::vector& update_loop) { update_loop.emplace_back(\[this\](float dt) { update(dt); }); } void update(float dt); }; int main() { std::vector update_loop; Wanderer wanderer{update_loop}; Diver diver{update_loop}; while (true) { for (auto & function : update_loop) { function(0.016f); } } } {% endhighlight %}

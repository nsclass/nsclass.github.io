...
{% highlight wl linenos %} class Invoker where T : class { static void Invoke(T invokeClass, string methodName, object\[\] args) { Debug.Assert(invokeClass != null); Type type = typeof(T); MethodInfo methodInfo = type.GetMethod(methodName); try { methodInfo.Invoke(invokeClass, args); } catch(Exception e) { Trace.WriteLine(e.Message); } } } {% endhighlight %}
